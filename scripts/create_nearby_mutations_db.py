...
import csv
import argparse
from biotite.structure.io.pdbx import CIFFile, BinaryCIFFile, get_structure

SCRIPT_DIR = Path(__file__).parent
PROJECT_ROOT = SCRIPT_DIR.parent
MODELS_ROOT = PROJECT_ROOT / "afdb_models"
...
def find_model_file(uniprot_dir):
    candidates = sorted(uniprot_dir.glob("*.cif")) + sorted(uniprot_dir.glob("*.bcif"))
    return candidates[0] if candidates else None


def load_first_chain(model_file):
    try:
        if model_file.suffix == ".bcif":
            cif = BinaryCIFFile.read(str(model_file))
        else:
...
with OUTPUT_PATH.open("w", encoding="utf-8", newline="") as handle:
    ...
    for uniprot_dir in sorted(MODELS_ROOT.iterdir()):
        ...
        if model_file is None:
            print(f"Skipping {uniprot}: no CIF/BCIF file found.")
            continue
